                ...
                <div class="card">
                    ...
                    <div
                        ...
                        onclick="openDetailModal(
                            ...
                            `{{ $booking->user->name }}`,
                            `{{ $booking->lapak->jenis }}`,
                            `{{ number_format($booking->lapak->harga,0,',','.') }}`,
                            `{{ \Carbon\Carbon::parse($booking->tanggal_booking)->translatedFormat('d F Y') }}`,
                            `{{ $booking->jam_booking }}`,
                            `{{ $booking->jumlah_orang }}`,
                            `{{ $booking->metode_pembayaran }}`,
                            `{{ number_format($booking->lapak->harga * $booking->jumlah_orang,0,',','.') }}`,
                            `{{ asset('bukti_tf/' . $booking->bukti_tf) }}`,
                            `{{ $booking->status }}`,
                    ...
                    <div class="card-body">
                        ...
                        <p>
                            <b>Harga:</b>
                            Rp {{ number_format($booking->lapak->harga,0,',','.') }}
                        </p>
    ...
    <div class="tab-content" id="confirmed">
        ...
        <div class="card-grid">
            ...
            @empty
                ...
                <div class="empty-message">
                    Belum ada transaksi dikonfirmasi
                </div>
...
function openDetailModal(
    bookingId, nama, user, jenis, harga, tanggal, jam,
    ...
    if(status === "pending") badge = `<span class="status pending">Menunggu</span>`;
    else if(status === "confirmed") badge = `<span class="status confirmed">Dikonfirmasi</span>`;
    else if(status === "completed") badge = `<span class="status completed">Selesai</span>`;
    else if(status === "rejected") badge = `<span class="status rejected">Ditolak</span>`;
    else if(status === "canceled") badge = `<span class="status canceled">Dibatalkan</span>`;
    ...
    if(status === "pending"){
        html = `
            <form action="/admin/booking/${bookingId}/confirm" method="POST">
                @csrf
                <button type="submit" class="btn">Konfirmasi</button>
            </form>

            <button type="button" class="btn reject-btn"
                onclick="openRejectModal('${bookingId}')">
                Tolak
            </button>
        `;
    }
        ...
        html = `
            <form action="/admin/booking/${bookingId}/complete" method="POST">
                @csrf
                <button type="submit" class="btn">Selesaikan</button>
            </form>
        `;
    }
...
function fetchTransaksi()
{
    ...
    .then(data => {
                ...
                <div class="card">
                    <div class="detail-icon"
                        onclick="openDetailModal(
                            ...
                            '${b.user.name}',
                            '${b.lapak.jenis}',
                            '${Number(b.lapak.harga).toLocaleString()}',
                            '${b.tanggal_booking}',
                            '${b.jam_booking}',
                            '${b.jumlah_orang}',
                            '${b.metode_pembayaran}',
                            '${Number(b.lapak.harga * b.jumlah_orang).toLocaleString()}',
                            '/bukti_tf/${b.bukti_tf}',
                            '${b.status}',
                            '${b.rejection_reason || ""}'
                        )">
        ...
        const activeTab = sessionStorage.getItem("activeTab") || currentTab || "pending";
